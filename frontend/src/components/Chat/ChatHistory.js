...
const ChatHistory = ({ onSubmit }) => {
  ...
  return (
    <div className="transform xl:translate-x-0 ease-in-out transition duration-500 flex justify-start items-start h-[calc(100vh-72px)] w-full sm:w-64 border-r border-gray-200 bg-white flex-col">
      <div className="flex flex-col justify-center items-center pl-5 pr-5 w-full pb-5">
      <ul>
      {chats.map((chat) => (
        <li key={chat.id}>
                <Link to={`/chat/${chat.id}`} className="w-64 h-15 mb-4 px-3 py-3 rounded-lg text-base text-gray-400 overflow-hidden hover:text-gray-300 focus:bg-gray-700 focus:text-white hover:bg-gray-700 flex jusitfy-start items-center" role="alert">
      ...
      <Link to="/chat" className="absolute bottom-1 w-64 h-15 mb-4 px-3 py-3 rounded-lg text-base overflow-hidden text-white bg-gray-700 focus:text-white hover:bg-gray-700 flex jusitfy-start items-center" role="alert">
